...
SELECT
    ...
    PatientList.Artnum, -- Added ART Number
    PatientList.type,
    PatientList.Sex,
    PatientList.Age, -- Added Age
    PatientList.LastVisitDate -- Added Last Visit Date
FROM (
    ...
    SELECT 
        ...
        FLOOR(DATEDIFF(@PreviousEndDate, main.DaBirth) / 365.25) as Age, -- Calculate Age
        pla.LastVisitDate
    FROM tblaimain main
    JOIN ( 
    ...
    WHERE NOT EXISTS (SELECT 1 FROM tblavpatientstatus s WHERE s.ClinicID = main.ClinicID AND s.Da <= @PreviousEndDate AND s.Status IN (@dead_code, @transfer_out_code, @lost_code)) 
    ...
    WHERE NOT EXISTS (SELECT 1 FROM tblcvpatientstatus s WHERE s.ClinicID = main.ClinicID AND s.Da <= @PreviousEndDate AND s.Status IN (@dead_code, @transfer_out_code, @lost_code)) 
      AND DATEDIFF(@PreviousEndDate, pla.DaApp) < 28
) AS PatientList;